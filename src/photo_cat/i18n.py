...
ITALIAN: dict[str, str] = {
    ...
    "Select filter transmission file": "Seleziona il file di trasmissione del filtro",
    "Filter files": "File di filtro",
    ...
    "Save and run pipeline": "Salva e avvia la pipeline",
    ...
    "Stellar parameters (optional, PHOENIX)": "Parametri stellari (opzionali, PHOENIX)",
    "One parameter=catalog_column per line, for example teff_gspphot_phoenix=teff_gspphot_phoenix. Missing row values use the recorded fallback.":
        "Un parametro=colonna_catalogo per riga, per esempio teff_gspphot_phoenix=teff_gspphot_phoenix. I valori mancanti usano il fallback registrato.",
    "Apply PHOENIX extinction from azero_gspphot_phoenix":
        "Applica l'estinzione PHOENIX da azero_gspphot_phoenix",
    "PHOENIX grid path": "Percorso griglia PHOENIX",
    "Extinction R_V": "Estinzione R_V",
    ...
    "Photometric band conversion (optional)": "Conversione fotometrica di banda (opzionale)",
    "Output band": "Banda di uscita",
    "A custom filter file is required only for the custom output band. Format: two columns 'wavelength transmission', optional headers '# unit: nm|angstrom|micron' and '# transmission: fraction|percent'.":
        "Un file di filtro personalizzato serve solo per la banda di uscita custom. Formato: due colonne 'wavelength transmission', header opzionali '# unit: nm|angstrom|micron' e '# transmission: fraction|percent'.",
    "Download a filter from SVO": "Scarica un filtro da SVO",
    "Facility": "Facility",
    "Filter": "Filtro",
    "List filters": "Elenca i filtri",
    "Download and use": "Scarica e usa",
    "Fetch any filter from the SVO Filter Profile Service. Pick a facility, list its filters, then download one; it is saved to the filter library and selected as the output band.":
        "Scarica qualsiasi filtro dal SVO Filter Profile Service. Scegli una facility, elenca i suoi filtri, poi scaricane uno; viene salvato nella libreria dei filtri e selezionato come banda di uscita.",
    "Choose a facility before listing filters.": "Scegli una facility prima di elencare i filtri.",
    "Listing filters from SVO...": "Elenco dei filtri da SVO in corso...",
    "Found {count} filters. Choose one and download it.": "Trovati {count} filtri. Scegline uno e scaricalo.",
    "Choose a filter to download.": "Scegli un filtro da scaricare.",
    "Downloading {filter} from SVO...": "Download di {filter} da SVO in corso...",
    "Downloaded {filter}. Output band set to {band}.": "Scaricato {filter}. Banda di uscita impostata a {band}.",
    "Conversion method": "Metodo di conversione",
    "Custom filter file": "File di filtro personalizzato",
    "Choose the catalogue and the output band used for contamination. If that exact band is stored in the index, PHOTO-CAT uses its nominal catalogue magnitude directly. Otherwise BP-RP supplies the temperature used to convert each source into the chosen band.":
        "Scegli il catalogo e la banda di uscita usata per la contaminazione. Se quella banda esatta e' presente nell'indice, PHOTO-CAT usa direttamente la sua magnitudine nominale di catalogo. Altrimenti BP-RP fornisce la temperatura usata per convertire ogni sorgente nella banda scelta.",
    "A custom filter file is required only for the custom output band. Format: two columns 'wavelength transmission', optional headers '# unit: nm|angstrom|micron' and '# transmission: fraction|percent'. Built-in mission bands with a nominal placeholder curve are indicative only until you install the official one.":
        "Un file di filtro personalizzato serve solo per la banda di uscita custom. Formato: due colonne 'wavelength transmission', header opzionali '# unit: nm|angstrom|micron' e '# transmission: fraction|percent'. Le bande di missione integrate con curva nominale segnaposto sono solo indicative finche non installi quella ufficiale.",
    ...
    "Replace running pipeline when Save and run is clicked": "Sostituisci la pipeline attiva quando si usa Salva + avvia",
    ...
    "Write an HTML, Markdown, or PDF report from a PHOTO-CAT query result JSON.":
        "Genera un report HTML, Markdown o PDF da un JSON di risultati PHOTO-CAT.",
    ...
    "Enabled: the previous pipeline window opened by this GUI is closed before a new run starts. Disabled: each Save and run opens a separate pipeline window.":
    ...
    "write an SVG, PNG, or PDF plot from a PHOTO-CAT query result JSON":
        "genera un grafico SVG, PNG o PDF da un JSON di risultati PHOTO-CAT",
    ...
    "write an HTML, Markdown, or PDF report from a PHOTO-CAT query result JSON":
        "genera un report HTML, Markdown o PDF da un JSON PHOTO-CAT",
    "PDF report generation requires matplotlib.": "La generazione di report PDF richiede matplotlib.",
    "PDF reports must be written to a file.": "I report PDF devono essere scritti in un file.",
    ...
    "plot output format; PNG and PDF use matplotlib": "formato del grafico; PNG e PDF usano matplotlib",
    "plot output path": "percorso del grafico di output",
...
TOOLTIPS_EN: dict[str, str] = {
    ...
    "Stellar parameters (optional, PHOENIX)": "Map Gaia atmospheric-parameter columns used to select and interpolate the local PHOENIX grid.",
    "One parameter=catalog_column per line, for example teff_gspphot_phoenix=teff_gspphot_phoenix. Missing row values use the recorded fallback.": "Enter the internal PHOENIX parameter key and its exact CSV column name; individual missing values remain valid fallback cases.",
    "Apply PHOENIX extinction from azero_gspphot_phoenix": "Redden each PHOENIX spectrum with its Gaia A0 value before normalizing it on Gaia G.",
    "PHOENIX grid path": "Local directory containing grid_index.csv and its referenced PHOENIX spectra.",
    "Extinction R_V": "CCM89 total-to-selective extinction ratio; 3.1 is the diffuse Milky Way default.",
    ...
    "Photometric band conversion (optional)": "Convert catalogue flux into a mission band before contamination is computed.",
    "Output band": "The band contamination is estimated in. Choose a built-in mission filter, a band with a published Gaia relation such as johnson_v, custom for your own curve, or none to keep the catalogue band.",
    "A custom filter file is required only for the custom output band. Format: two columns 'wavelength transmission', optional headers '# unit: nm|angstrom|micron' and '# transmission: fraction|percent'.": "Provide a custom filter only for the custom band; two columns, wavelength and transmission.",
    "Download a filter from SVO": "Pull any filter from the SVO Filter Profile Service into the local library.",
    "Facility": "The mission or observatory whose filters to list, for example TESS or JWST.",
    "Filter": "The specific filter to download from the chosen facility.",
    "List filters": "Query SVO for every filter the chosen facility provides.",
    "Download and use": "Download the selected filter and set it as the output band.",
    "Fetch any filter from the SVO Filter Profile Service. Pick a facility, list its filters, then download one; it is saved to the filter library and selected as the output band.": "One filter at a time is downloaded, saved to the library, and made selectable immediately.",
    "Conversion method": "How flux is converted between bands. blackbody integrates a spectrum through the filter and works for any curve; gaia_empirical applies a published Gaia colour relation, for the systems it covers; auto uses the relation where it is calibrated and blackbody elsewhere. phoenix needs external data.",
    "Choose the catalogue and the output band used for contamination. If that exact band is stored in the index, PHOTO-CAT uses its nominal catalogue magnitude directly. Otherwise BP-RP supplies the temperature used to convert each source into the chosen band.": "An exact stored output band always wins; colour conversion is used only when that catalogue measurement is unavailable.",
    "A custom filter file is required only for the custom output band. Format: two columns 'wavelength transmission', optional headers '# unit: nm|angstrom|micron' and '# transmission: fraction|percent'. Built-in mission bands with a nominal placeholder curve are indicative only until you install the official one.": "Provide a custom filter only for the custom band. Nominal placeholder curves are indicative until replaced with the official response.",
...
TOOLTIPS_IT: dict[str, str] = {
    ...
    "Stellar parameters (optional, PHOENIX)": "Associa le colonne dei parametri atmosferici Gaia usate per selezionare e interpolare la griglia PHOENIX locale.",
    "One parameter=catalog_column per line, for example teff_gspphot_phoenix=teff_gspphot_phoenix. Missing row values use the recorded fallback.": "Inserisci la chiave interna del parametro PHOENIX e il nome esatto della colonna CSV; i singoli valori mancanti restano casi di fallback validi.",
    "Apply PHOENIX extinction from azero_gspphot_phoenix": "Arrossa ogni spettro PHOENIX con il valore Gaia A0 prima di normalizzarlo su Gaia G.",
    "PHOENIX grid path": "Cartella locale contenente grid_index.csv e gli spettri PHOENIX referenziati.",
    "Extinction R_V": "Rapporto CCM89 tra estinzione totale e selettiva; 3.1 è il valore predefinito per la Via Lattea diffusa.",
    ...
    "Photometric band conversion (optional)": "Converte il flusso del catalogo in una banda di missione prima del calcolo della contaminazione.",
    "Output band": "La banda in cui si stima la contaminazione. Scegli un filtro di missione integrato, una banda con relazione Gaia pubblicata come johnson_v, custom per una curva tua, o nessuna per la banda del catalogo.",
    "A custom filter file is required only for the custom output band. Format: two columns 'wavelength transmission', optional headers '# unit: nm|angstrom|micron' and '# transmission: fraction|percent'.": "Fornisci un filtro personalizzato solo per la banda custom; due colonne, lunghezza d'onda e trasmissione.",
    "Download a filter from SVO": "Scarica qualsiasi filtro dal SVO Filter Profile Service nella libreria locale.",
    "Facility": "La missione o osservatorio di cui elencare i filtri, per esempio TESS o JWST.",
    "Filter": "Il filtro specifico da scaricare dalla facility scelta.",
    "List filters": "Interroga SVO per tutti i filtri forniti dalla facility scelta.",
    "Download and use": "Scarica il filtro selezionato e impostalo come banda di uscita.",
    "Fetch any filter from the SVO Filter Profile Service. Pick a facility, list its filters, then download one; it is saved to the filter library and selected as the output band.": "Si scarica un filtro alla volta, viene salvato nella libreria e reso subito selezionabile.",
    "Conversion method": "Come si converte il flusso tra bande. blackbody integra uno spettro attraverso il filtro e vale per qualsiasi curva; gaia_empirical applica una relazione di colore Gaia pubblicata, per i sistemi che copre; auto usa la relazione dove e' calibrata e blackbody altrove. phoenix richiede dati esterni.",
    "Choose the catalogue and the output band used for contamination. If that exact band is stored in the index, PHOTO-CAT uses its nominal catalogue magnitude directly. Otherwise BP-RP supplies the temperature used to convert each source into the chosen band.": "Una banda di uscita esatta gia' presente ha sempre la precedenza; la conversione cromatica si usa solo quando quella misura di catalogo manca.",
    "A custom filter file is required only for the custom output band. Format: two columns 'wavelength transmission', optional headers '# unit: nm|angstrom|micron' and '# transmission: fraction|percent'. Built-in mission bands with a nominal placeholder curve are indicative only until you install the official one.": "Fornisci un filtro personalizzato solo per la banda custom. Le curve nominali segnaposto sono indicative finche non le sostituisci con la risposta ufficiale.",
...
TOOLTIPS_EN.update({
    ...
    "Output directory": "Folder that will receive all generated files. For result-based products it is auto-filled beside the result JSON with a descriptive directory name.",
    "Output data file (path)": "Destination filename for an exported CSV or Parquet table. It is auto-filled beside the selected result and follows the Format choice until edited manually.",
    ...
    "Output summary file (optional; prints to console if blank)": "Destination for the summary, auto-filled beside the result with a .txt, .json, or .csv extension matching Format. Clear it to print only to the console.",
    "Output screening file (path)": "Destination for ranked decisions. The automatic filename follows the selected CSV, JSON, or Markdown format and never replaces a manually entered path.",
    ...
    "Report": "Combines a human-readable summary and plots into HTML, Markdown, or a multi-page PDF. Use PDF for sharing or submission, HTML for local viewing, and Markdown for repositories.",
...
TOOLTIPS_IT.update({
    ...
    "Output directory": "Cartella che riceverà i file generati. Per i prodotti basati sui risultati viene compilata accanto al JSON con un nome descrittivo.",
    "Output data file (path)": "File di destinazione CSV o Parquet, compilato accanto al risultato e aggiornato in base a Formato finché non lo modifichi manualmente.",
    ...
    "Output summary file (optional; prints to console if blank)": "Destinazione del riepilogo, compilata accanto al risultato con estensione .txt, .json o .csv coerente con Formato. Svuotala per stampare solo in console.",
    "Output screening file (path)": "Destinazione delle decisioni, con nome automatico coerente con CSV, JSON o Markdown e senza sostituire percorsi inseriti manualmente.",
    ...
    "Report": "Combina riepilogo e grafici in HTML, Markdown o PDF multipagina. Usa PDF per condivisione o invio, HTML per consultazione locale e Markdown per repository.",
...
TOOLTIPS_EN.update({
    ...
    "Plot": "Open diagnostic plotting. Choose the scientific relationship and export it as SVG, PNG, or PDF; PNG and PDF use matplotlib automatically.",
    ...
    "Replace running pipeline when Save and run is clicked": "When enabled, closes the previous pipeline process started by this GUI before launching another, preventing accidental duplicate heavy runs.",
    ...
    "Save and run pipeline": "Validates and saves config.yaml, then launches the selected build/query stages in a separate console. Large catalogues may take substantial time and disk space.",
    ...
    "Write an HTML or Markdown report from a PHOTO-CAT query result JSON.": "Builds a portable summary. HTML embeds SVG plots, Markdown suits repositories, and PDF creates a summary page followed by diagnostic plot pages.",
    "Write an HTML, Markdown, or PDF report from a PHOTO-CAT query result JSON.": "Builds a portable summary. HTML embeds SVG plots, Markdown suits repositories, and PDF creates a summary page followed by diagnostic plot pages.",
...
TOOLTIPS_IT.update({
    ...
    "Plot": "Apre i grafici diagnostici. Scegli la relazione scientifica ed esportala come SVG, PNG o PDF; PNG e PDF usano automaticamente matplotlib.",
    ...
    "Replace running pipeline when Save and run is clicked": "Chiude la precedente pipeline avviata dalla GUI prima di lanciarne un'altra, evitando due elaborazioni pesanti accidentali.",
    ...
    "Save and run pipeline": "Valida e salva config.yaml, poi avvia build/query in una console separata. Cataloghi grandi possono richiedere tempo e spazio.",
    ...
    "Write an HTML or Markdown report from a PHOTO-CAT query result JSON.": "Crea un riepilogo portabile. HTML incorpora SVG, Markdown è adatto ai repository e PDF aggiunge pagine diagnostiche dopo il riepilogo.",
    "Write an HTML, Markdown, or PDF report from a PHOTO-CAT query result JSON.": "Crea un riepilogo portabile. HTML incorpora SVG, Markdown è adatto ai repository e PDF aggiunge pagine diagnostiche dopo il riepilogo.",
