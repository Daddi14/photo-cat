...
Il modello attuale di PHOTO-CAT 3.1.0 ha un ambito volutamente definito: usa magnitudini di catalogo, un'apertura circolare e una PSF gaussiana circolare 2D opzionale, il cui flusso decade con la distanza angolare dal centro dell'apertura. Un raggio di influenza ricavato dalla larghezza della PSF può includere il leakage pesato di sorgenti vicine fuori dall'apertura. Una conversione di banda opzionale può stimare la contaminazione in un'altra banda con un corpo nero, una SED PHOENIX/BT-Settl locale interpolata e normalizzata su Gaia G, oppure una trasformazione empirica Gaia pubblicata nel proprio intervallo calibrato. Non esegue convoluzione con PSF asimmetriche o variabili, modellazione dei pixel o luce diffusa. Considera l'output come metrica di screening/rischio salvo che input di missione calibrati supportino i modelli selezionati.
...
- Converte il flusso del catalogo in un'altra banda prima del calcolo della contaminazione: corpo nero, fotometria sintetica PHOENIX/BT-Settl locale in Teff/logg/[M/H], trasformazione empirica Gaia pubblicata, oppure `auto` per usare la relazione dove è calibrata e il corpo nero altrove.
