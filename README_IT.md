...
Il modello attuale di PHOTO-CAT 3.1.0 ha un ambito volutamente definito: usa magnitudini di catalogo, un'apertura circolare e una PSF gaussiana circolare 2D opzionale, il cui flusso decade con la distanza angolare dal centro dell'apertura. Un raggio di influenza ricavato dalla larghezza della PSF può includere il leakage pesato di sorgenti vicine fuori dall'apertura. Una conversione di banda opzionale può stimare la contaminazione in un'altra banda a partire dai colori del catalogo: una conversione corpo-nero colore→banda per le bande di missione (TESS, CHEOPS, MAUVE, Ariel o qualsiasi filtro dal servizio SVO), oppure una trasformazione empirica Gaia pubblicata per i sistemi fotometrici su cui è calibrata (Johnson-Cousins, 2MASS, SDSS, Hipparcos, Tycho-2), entro i loro intervalli di colore pubblicati. Non esegue convoluzione con PSF asimmetriche o variabili, modellazione dei pixel, luce diffusa o integrazione spettrale completa sulla banda. Considera l'output come metrica di screening/rischio salvo che input di missione calibrati supportino i modelli selezionati.
...
- Converte il flusso del catalogo in un'altra banda prima del calcolo della contaminazione: conversione corpo nero colore→banda attraverso qualsiasi curva di trasmissione (filtri integrati o scaricati dal servizio SVO nella GUI), trasformazione empirica Gaia pubblicata per i sistemi che copre, oppure `auto` per usare la relazione dove è calibrata e il corpo nero altrove.
