...
When a photometric conversion was used, screen or validate its mission-band
estimate with `--metric flux_fraction_total_weighted_converted`.
...
| `build_neighbors_index.io.magnitude_columns` | `--magnitude-columns band=column,...` |
| `build_neighbors_index.io.stellar_parameter_columns` | `--stellar-parameter-columns parameter=column,...` |
